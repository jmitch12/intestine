...
void draw() {
  ...
  if (analogValue > 600){
    ...
    jane.analogWrite(ledPin2, Arduino.HIGH);
    jane.digitalWrite(ledPin3, Arduino.LOW);
    delay(100);
    } else {                           
     jane.digitalWrite(ledPin, Arduino.LOW); 
     jane.analogWrite(ledPin2, Arduino.LOW); 
...
void renderWave() {
  ...
  for (int x = 0; x < yvalues.length; x++) {
    ellipse(x*xspacing, height/2.3+yvalues[x], 16, 16);
  }
  ...
  for (int x2 = 0; x2 < yvalues2.length; x2++) {
    ellipse(x2*xspacing2, height/1.6+yvalues2[x2], 16, 16);
  }
}